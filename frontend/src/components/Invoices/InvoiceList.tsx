  ...
  if (invoices.length === 0) {
    return (
      <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-8 text-center">
        ...
        <h2 className="text-xl font-semibold text-[var(--foreground)] mb-2">No Invoices Yet</h2>
          ...
          <tbody>
            {filteredInvoices.map((invoice) => (
              ...
              >
                ...
                <td className="px-6 py-4">
                  <div>
                    <p className="text-sm text-[var(--foreground)]">{invoice.clientName}</p>
                    <p className="text-xs text-[var(--muted-foreground)]">{invoice.clientEmail}</p>
                  </div>
                </td>
        ...
        {filteredInvoices.map((invoice) => (
          ...
          >
            <div className="flex items-start justify-between mb-3">
              <div>
                <p className="font-medium text-[var(--foreground)]">{invoice.invoiceNo}</p>
                <p className="text-sm text-[var(--muted-foreground)]">{invoice.clientName}</p>
              </div>
              <span
                className={`px-2.5 py-1 rounded-full text-xs font-medium capitalize ${getStatusStyles(
                  invoice.status
                )}`}
              >
                {invoice.status}
